...
def get_label_encoder(name: str,
    ...
    """
    repo = Repository()
    training_pipeline = repo.get_pipeline(pipeline_name=pipeline_name)
    return training_pipeline.runs[-1].get_step(name=step_name).outputs[
        name].read()

...
@step
def feature_engineer(dataset: pd.DataFrame,
                       config: FeatureEngineererConfig) -> pd.DataFrame:
    ...
    def add_opponent(match_rows):
        ...
        match_rows['OPPONENT_TEAM_ABBREVIATION'] = match_rows.apply(lambda x: opponent_dir[x['TEAM_ABBREVIATION']], axis=1)
        return match_rows

    select_features = config.select_features + ['GAME_ID']

    dataset = limit_timeframe(dataset=dataset,
                              years_to_subtract=config.history_length)

    return dataset[select_features].groupby('GAME_ID').apply(add_opponent).drop(columns=['GAME_ID'])
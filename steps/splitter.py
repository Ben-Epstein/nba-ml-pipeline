...
def sklearn_splitter(
...
) -> Output(train_x=pd.DataFrame, train_y=pd.DataFrame,
    ...
    dataset_x, dataset_y = split(
        dataset, config.target_col)
...
@step
def date_based_splitter(dataset: pd.DataFrame,
                        config: SplitConfig) -> Output(before=pd.DataFrame,
                                                       after=pd.DataFrame):
    ...
    return dataset[dataset['GAME_DATE'] < config.date_split][cols], \
           dataset[dataset['GAME_DATE'] >= config.date_split][cols]


class TrainingSplitConfig(BaseStepConfig):
    new_data_split_date: str  # date to split on
...
def reference_data_splitter(
    ...
    reference_dataset = dataset.loc[dataset['GAME_DATE'].between(
        config.start_reference_time_frame, config.end_reference_time_frame,
        inclusive=True)][cols]
...
@step
def get_coming_week_data(dataset: pd.DataFrame,
                         config: TimeWindowConfig) -> pd.DataFrame:
